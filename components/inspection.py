...
def render_inspection():
    ...
    logger.info("Renderizando Inspección")
    ...
    with tab2:
        ...
        if total_nulls == 0:
            ...
            st.warning(f"Se encontraron {total_nulls:,} valores nulos en {len(missing[missing['Nulos']>0])} columnas")
            ...
            with col1:
                ...
                st.dataframe(cols_with_nulls[['Columna', 'Nulos', '% Nulos']], width='stretch')
            with col2:
    ...
    with tab3:
        ...
        if duplicates == 0:
            ...
            if st.button("Mostrar filas duplicadas"):
                duplicated_mask = df.duplicated(keep=False)
                st.dataframe(df[duplicated_mask], width='stretch')
    ...
    with tab4:
        ...
        if numeric_cols:
            ...
            if n_outliers > 0:
                ...
                st.plotly_chart(fig, width='stretch')
                with st.expander("Ver registros con outliers"):
                    st.dataframe(df[outliers_mask], width='stretch')
            else:
