...
def render_cleaning():
    ...
    with col_main:
        ...
        if duplicates > 0:
            ...
            if st.button("Eliminar todas las filas duplicadas", type="primary"):
        ...
        if cols_with_nulls:
            selected_col = st.selectbox("Selecciona una columna con valores nulos", cols_with_nulls, key="null_col")
            ...
            strategy = st.radio("Estrategia de imputación", list(IMPUTATION_STRATEGIES.keys()), horizontal=True, key="impute_strategy")
            if st.button("Aplicar limpieza a esta columna", type="primary"):
                df = DataCleaner.fill_missing(df, selected_col, IMPUTATION_STRATEGIES[strategy])
                st.session_state.df = df
        ...
        col_to_change = st.selectbox("Seleccionar columna para convertir", df.columns.tolist(), key="convert_col")
        current_type = st.session_state.col_types.get(col_to_change, "desconocido")
        st.info(f"Tipo actual detectado: **{current_type}**")
        new_type = st.selectbox("Convertir a", ["numérica", "texto", "fecha", "categórica"], key="new_type")
        ...
        if st.button("Guardar todos los cambios y continuar", type="primary", width='stretch'):
            st.session_state.df = df
